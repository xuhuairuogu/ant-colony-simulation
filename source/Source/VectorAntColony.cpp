...
VectorAntColony::VectorAntColony(void) :
	AntColony()
{
}
...
VectorAntColony::VectorAntColony(uint BugTexture, Grid* grid, FoodManager* food_manager) :
	AntColony(BugTexture, grid, food_manager)
{
}

void VectorAntColony::Update(float delta_time)
{
	AntColony::Update(delta_time);
}

VectorAnt* VectorAntColony::AddVectorAnt(int2 pos)
{
	VectorAnt* new_ant = new VectorAnt(m_bug_texture, 0.0f, m_grid, m_home, m_food_manager);
	new_ant->SetTarget(pos + int2(rnd(-3, 3),rnd(-3, 3)));
	if (AddAnt(new_ant, pos)) return new_ant;
	else
	{
		delete new_ant;
		return 0;
...
void VectorAntColony::AddAntsNearHome(uint count)
{
	for (uint i = 0; i < count; ++i) 
	{
		AddVectorAnt(m_home->GetPos());
	}
}